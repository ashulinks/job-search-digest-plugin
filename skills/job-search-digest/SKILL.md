...
5. **Output template** — single-select: "Use the default template" vs. "I'll provide my own template," per `references/digest-format.md`'s "Output template preference." Ask this once, like the others — not on every run. If the user picks their own, ask for the file (free text/upload, not a click choice) unless `Job Search/User Template.docx` already exists in Drive, and save it there. Every future run reads the stored `Template:` choice from `config.md` and passes it straight to `cv-jd-tailor` without asking again — cv-jd-tailor's own Step 1 template question only fires when it's invoked directly, outside this pipeline.
6. **Delivery** — single-select: email / push notification (if running via Cowork Scheduled Task) / stored as a Claude artifact only.
...
Save the resulting config (keywords, countries, boards by country, delivery, cadence, output template preference) to Drive as the single global `config.md` per the storage convention in `references/digest-format.md` — one file, not one per country — so re-runs and scheduled runs don't need to re-ask.
...
- **MCP-backed boards** (none by default currently — see `references/job-boards.md`): if a genuinely relevant connector is later added to the registry, call its job-search tool directly with the keyword set and country/location filter instead of the web-fallback method above.
...
For each ≥7 match, invoke the `cv-jd-tailor` skill with: the master profile, the fetched JD, the target country, the user's citizenship/work-rights status for that country (ask once, reuse thereafter), and the output template per `config.md`'s stored `Template:` preference (Step 1) — either way, hand it the per-country `Base CV.docx` (`references/digest-format.md`'s "Base CV cache"), not the raw `CV_Template.docx` or `User Template.docx` directly, except when that cache needs rebuilding this run. Never ask cv-jd-tailor's own Step 1 template question during this pipeline — the answer already came from Step 1 above. Respect cv-jd-tailor's own Step 5 gate — if its projected score comes in under 8/10 despite this pre-filter, let it stop and report the gap rather than forcing a document; put that outcome in the digest's "Attempted, not tailored" section (`references/digest-format.md`) rather than under "Tailored", and log it with outcome `gap` in Step 8. No cap on how many matches get tailored per run — every ≥7 match is tailored — but the Base CV cache (`references/digest-format.md`) applies equally to the default and a user-supplied template, so only the first tailoring per country per (template, profile) version pays the full document-build cost; every subsequent one this run or a later run reuses that cached base.

cv-jd-tailor saves every CV it produces to Drive itself, using this exact folder convention, as its own Step 8 — this pipeline never needs to duplicate that save; just point this run's digest ("Tailored CV:" line, Step 7) at the same `Job Search/<Country>/Tailored CVs/YYYY-MM-DD_Company_RoleTitle.docx` path.
