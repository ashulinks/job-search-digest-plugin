...
Unless Step 1 already established a user-supplied template, pick the starting document per `references/templates.md`'s "How to use the asset template" (which defers to `job-search-digest/references/digest-format.md`'s "Base CV cache"): a current `<Country>/Base CV.docx` if one exists and matches the profile, otherwise `assets/templates/CV_Template.docx` — the single standard template shared by all three countries. This is the literal starting document for docx output, always: never regenerate from scratch (e.g. with docx-js) when a starting document exists — edit a copy of it in place per `references/templates.md`'s instructions, replacing every remaining bracketed placeholder with the candidate's real content.
...
- `references/templates.md` — the single standard template shared by all three countries, its structure, tone/voice, domain-filling rules, the asset-file editing workflow, and the Base CV cache that skips redundant placeholder-filling across repeat runs
