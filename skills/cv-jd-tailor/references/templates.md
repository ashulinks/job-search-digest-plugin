...
`CV_Template.docx` is the base asset for Australia, Singapore, and UAE alike — there is no per-country file to choose between. Country-specific deltas (work-rights status wording, regulatory framework ordering, the optional photo) are applied at edit time per `country-rules.md` and the "Per-market notes" below, not by picking a different asset. In practice, most runs start one step further along than the raw asset — see "How to use the asset template" below for the Base CV cache that skips redundant reconciliation work.

A user-supplied template overrides which asset this is (SKILL.md Step 1) — but not whether the Base CV cache applies. When invoked from job-search-digest's pipeline with `config.md`'s `Template: user-supplied`, the supplied file (`Job Search/User Template.docx`) is a persisted, reused asset exactly like `CV_Template.docx`, so it gets the same caching treatment (`digest-format.md`'s "Base CV cache"). Only a genuinely one-off template supplied for a single standalone cv-jd-tailor run — outside the digest pipeline, with nowhere persisted to cache it against — is edited fresh every time with no Base CV involved.
...
1. **Pick the starting document per `digest-format.md`'s "Base CV cache"** — this applies whether the configured source is the default `CV_Template.docx` or a persisted user-supplied `Job Search/User Template.docx`: if a current `<Country>/Base CV.docx` exists (its `Base CV.meta.md` fingerprints match both the current template source and `cv-profile.md`), copy that — every placeholder is already filled and/or every fact already reconciled with the candidate's real content, so this run only needs Step 5's JD-specific reordering/trimming/relabeling on top of it. Otherwise copy the current template source (`CV_Template.docx` or `User Template.docx`), do the full reconciliation pass this run, and save the result back as the new `Base CV.docx` with updated fingerprints (per `digest-format.md`) before continuing with JD-specific tailoring. For a genuinely one-off template supplied for a single standalone run with nothing persisted to cache against, skip this step and just work from that file directly. Either way, never start from a blank document.
2. Follow the docx skill's "Editing existing documents" flow: unzip, run `merge_runs.py`, then make **every** edit for this run — placeholder-filling if starting from the raw template, JD-specific reordering/trimming/relabeling, row-height fixes, keep-with-next fixes, image insertion, all of it — as **direct XML edits inside the unzipped `word/document.xml`** (and, only for an inserted image, `word/media/`, `word/_rels/document.xml.rels`, and `[Content_Types].xml`), scripted in **one Python script executed in a single Bash call**, not a series of separate `Edit`/`Bash` round trips. Each additional tool-call round repeats fixed per-turn overhead on top of the edit itself, so planning the full set of changes before touching the file and applying them in one pass is meaningfully cheaper than iterating edit-by-edit.
   **Never open the working file with python-docx's `Document(path)` and call `.save()` on it, at any point in this workflow, for any of the edits below (row height, keep-with-next, image insertion included).** That performs a full package rewrite through python-docx's object model, which doesn't round-trip every element of a hand-authored template faithfully — it is the confirmed cause of a real incident where a tailored CV lost all 8 table-shading regions and the great majority of run-level text colors from the source template, while gaining a `stylesWithEffects.xml` and `docProps/thumbnail.jpeg` that don't exist in `CV_Template.docx` (both telltale signs of a full re-save rather than an in-place XML edit). Manipulate the string/tree content of `word/document.xml` directly instead, the same way you edit any other text in this step.
...
5. Only build from scratch if the user asks for a non-docx format, or supplies their own template in a format that can't be edited in place the same way (steps 1–4 above apply to a user-supplied docx template exactly as they do to the default one, whether it's a persisted cache-eligible file or a genuine one-off) — and disclose that deviation (SKILL.md's "never fail silently").
...
**Page 1 table with empty space.** Editing (reordering/trimming bullets, relabeling headers, condensing or expanding a column) routinely leaves one column shorter than the others in the capability matrix or the Education/Certifications/Memberships table. If that leaves visible blank space at the bottom of a row or table on page 1 — the row is taller than its content needs — resize it down rather than leaving the gap: find that row's `<w:trPr>` in `document.xml` and check its `<w:trHeight w:val="..." w:hRule="..."/>` against the tallest cell's actual content after edits, then either lower `w:val` to fit or change `w:hRule` to `"auto"` (or remove the `w:trHeight` element entirely) so the row hugs its content instead of holding a fixed/exact height it no longer needs. Re-render to confirm the gap is gone and no new page-break issue was introduced.

**Shading/color integrity check (mandatory, every run).** Before the render, confirm the edit didn't strip the template's own formatting — this is a real, previously-observed failure mode where a delivered CV rendered in plain black-and-white with none of the source template's navy header banner, blue accents, or tinted table columns. Compare element counts between the starting document (`Base CV.docx`, `CV_Template.docx`, or `User Template.docx` — whichever this run started from) and the edited `document.xml`:

```bash
for f in start.xml edited.xml; do echo "$f:"; grep -o 'w:shd ' "$f" | wc -l; grep -o '<w:color ' "$f" | wc -l; done
```

The `w:shd` count (table/cell shading) must match exactly — every edit in this workflow only changes text content and paragraph/run properties like `w:keepNext`, never a table's shading definitions, so any drop here means something other than a plain XML text edit touched the file. The `w:color` count should be in the same range, not a sharp drop — some reduction is expected if content was trimmed, but a fall to near-zero is not. If either check fails, do not deliver: the edit path used this run broke the "edit in place" rule above (most likely a python-docx `Document().save()` round-trip slipped in) — redo the edit as a direct `document.xml` text/tree edit against the original starting document instead of trying to patch the broken output.
...
Per `country-rules.md`, a UAE candidate's photo is included only if their own existing CV already has one — never add one that wasn't there. When one applies, add a third header-table column and insert the image directly in XML rather than assuming a baked-in photo column exists — there isn't one in the shared asset: add the image file under `word/media/`, add its relationship entry to `word/_rels/document.xml.rels`, add a `<w:drawing>`/`<a:blip>` element referencing that relationship inside the new cell's paragraph in `document.xml`, and rebalance the three column widths (`w:gridCol`/`w:tcW`). Fixed per market otherwise, not a per-run judgment call. Status line: Australia states citizenship/visa only ("Australian Citizen"); Singapore and UAE add a work-rights clause ("· Open to relocation, visa sponsorship required," "Singapore PR," "UAE Resident Visa (employer-sponsored, transferable)").
